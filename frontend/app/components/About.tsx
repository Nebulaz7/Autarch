...
export default function About() {
  ...
  return (
    <section
      id="pipeline"
      className="py-20 border-t border-border-color bg-background transition-colors duration-300"
    >
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="max-w-xl mb-16">
          ...
          <h2 className="font-syne text-3xl sm:text-4xl font-normal leading-tight text-foreground tracking-tight mt-2">
            How Autarch executes secure on-chain payouts
          </h2>
          <p className="font-dm-sans text-sm text-zinc-500 mt-4 leading-relaxed">
            By combining decentralized smart contracts on Somnia Network with
            consensus-backed web crawlers and LLM analysis, payouts require no
            human overhead, keeping agreements secure and rapid.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((item, idx) => (
            <div
              key={idx}
              className="p-8 bg-card-bg thin-border rounded-lg relative flex flex-col justify-between h-96 group hover:border-zinc-400 dark:hover:border-zinc-600 transition-all"
            >
              <div>
                {/* Header */}
                ...
                <h3 className="font-syne text-xl font-medium text-foreground mt-4 mb-2">
                  {item.title}
                </h3>
        ...
        <div className="mt-12 text-foreground p-6 rounded-lg border border-foreground bg-background flex flex-col sm:flex-row items-start gap-4">
          <div className="p-2 rounded bg-amber-100 text-amber-800 dark:bg-zinc-900 dark:text-amber-500 mt-0.5">
            <ShieldAlert className="w-5 h-5" />
          </div>
          <div>
            <h4 className="text-sm font-semibold text-foreground">
              Human-in-the-Loop Arbiter Override
            </h4>
            <p className="text-xs text-zinc-500 mt-1 leading-relaxed">
              If an AI evaluation returns a Fail verdict, developers can
              instantly trigger a formal dispute within a 24-hour window. This
              pauses finalization and alerts the designated human Arbiter (EVM
              multisig fallback) to manually review visual logs and override
              payouts as needed.
            </p>
          </div>
        </div>
